...
const Deserts = () => {
  ...
  return (
    <div>
      <Heading heading={"Majestic Deserts Of Pakistan"} />
      <div className="container mx-auto px-4 py-10">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-8">
          {Object.values(desertDestinations).map((region) =>
            region.destinations.map((destination, index) => (
              ...
              >
                ...
                <div className="relative">
                  ...
                  />
                  <div className="absolute top-3 left-3 bg-yellow-600 text-white px-3 py-1 text-sm font-semibold rounded-lg shadow">
                    ...
                    <ul className="mt-2 space-y-2 text-gray-600">
                      {destination.attractions.map((attraction, i) => (
                        <li key={i} className="flex items-center gap-2">
                          ...
                          {attraction.toLowerCase().includes("sun") && (
                            <FaSun className="text-yellow-500" />
                          )}
                          {attraction.toLowerCase().includes("camp") && (
                            <FaCampground className="text-green-600" />
                          )}
                          {attraction.toLowerCase().includes("caravan") && (
                            <FaCaravan className="text-blue-500" />
                          )}
                          {attraction}
                        </li>
                      ))}
